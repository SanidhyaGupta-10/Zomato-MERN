...
const PartnerLogin: FC = () => {
    ...
    const { loginFoodPartner, isLoadingPartner, error: authError } = useAuth()
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
        ...
        const success = await loginFoodPartner({
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-200 px-4">
            <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-8">
                <h1 className="text-2xl font-semibold text-gray-900 mb-1">Partner Login</h1>
                <p className="text-sm text-gray-500 mb-6">Login to your restaurant account</p>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    >
                        {isLoadingPartner ? 'Logging in...' : 'Login'}
                    </button>
                </form>

                <p className="text-center text-sm text-gray-600 mt-4">
                    Don't have an account?{' '}
                    <Link to="/food-partner/register" className="text-black font-semibold hover:underline">
                        Sign up
                    </Link>
                </p>
            </div>
